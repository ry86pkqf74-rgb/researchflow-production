 ...
 */
function formatRedaction(type: PhiFinding['type']): string {
 ...
 */
export function scrubObject<T = Record<string, unknown>>(
  obj: T,
  ...
  if (Array.isArray(obj)) {
    return obj.map(item => scrubObject(item, visited)) as T;
  }
 ...
 */
export function containsPhi(message: string): boolean {
  if (!message || typeof message !== 'string') {
    return false;
  }
 ...
 */
export function getPhiStats(message: string): Record<string, number> {
