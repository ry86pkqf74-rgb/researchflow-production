...
class OpenAIProvider:
    """
    LLM provider for OpenAI-compatible chat completion models.
    ...
    - If ``cfg.no_network`` is true, all OpenAI requests are blocked and a
      ``RuntimeError`` is raised.
    - If ``cfg.mock_only`` is true, real OpenAI requests are also blocked
      and a ``RuntimeError`` is raised.

    Required environment variables:

    - ``OPENAI_API_KEY`` (required): bearer token used for authentication.
      If it is not set, this provider fails closed by raising a
      ``RuntimeError`` instead of attempting a network call.
    - ``OPENAI_BASE_URL`` (optional): base URL for the OpenAI-compatible API.
    ...
    def generate_text(
        ...
        *,
        ...
        """
        Generate a chat completion from the OpenAI API with governance-aware network gating.

        This method constructs a chat-completions request using the provided prompts and model,
        sends it to the configured OpenAI endpoint, and wraps the response text and token usage
        ...
        model:
            The OpenAI chat model identifier to invoke.
        temperature:
        ...
        RuntimeError
            ...
            any outbound request. Also raised if the ``OPENAI_API_KEY`` environment
            variable is not set, or if the OpenAI API returns an error response.
        urllib.error.URLError
            If network connection fails (DNS resolution, connection refused, timeout).
        urllib.error.HTTPError
            If the OpenAI API returns an HTTP error status code.
        ...
        POST request to the OpenAI ``/chat/completions`` endpoint using the configured
        ...
        if cfg.no_network or cfg.mock_only:
            raise RuntimeError(
                "NO_NETWORK=1 or MOCK_ONLY=1 blocks OpenAI calls (fail-closed)."
            )
        ...
        try:
            ...
            raise RuntimeError(f"OpenAI API error (HTTP {e.code}): {error_msg}") from e
        except urllib.error.URLError as e:
            raise RuntimeError(f"Network error accessing OpenAI API: {e.reason}") from e
        ...
        usage = LLMUsage(
            input_tokens=safe_int(usage_p.get("prompt_tokens")),
            output_tokens=safe_int(usage_p.get("completion_tokens")),
        )
        return LLMResult(
            text=text,
            provider="openai",
