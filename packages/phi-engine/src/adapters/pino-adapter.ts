...
// Type definitions for Pino (to avoid requiring pino as a dependency)
 ...
 */
function scrubLogEntry(obj: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function createScrubbedLogger(options: PinoOptions = {}): Logger {
  ...
  try {
    ...
    throw new Error(
      'Pino is required to use createScrubbedLogger. Install it with: npm install pino'
    );
  }
  ...
  const wrapLogMethod = (method: LogFn): LogFn => {
    return ((...args: any[]) => {
      ...
      return (method as any).apply(baseLogger, args);
    }) as LogFn;
  };
